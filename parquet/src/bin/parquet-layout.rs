...
#[derive(Serialize, Debug)]
struct FileSize {
    row_groups: Vec<RowGroupSize>,
}

#[derive(Serialize, Debug)]
struct RowGroupSize {
    column_chunk_size: Vec<i64>,
}
...
impl Args {
    fn run(&self) -> Result<()> {
                    ...
                    column_chunk_size: r
                        ...
                        .map(|chunk| {
                            chunk
                                .pages
                                .iter()
                                .map(|page| page.compressed_bytes as i64 + page.header_bytes as i64)
                                .sum()
                        })
                        .collect(),
                })
        ...
        serde_json::to_writer_pretty(writer, &file_size).unwrap();
        Ok(())
    }
}
